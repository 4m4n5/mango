...
- Every asynchronous launcher Play emits one bounded `play_request_terminal` outcome (`playing`, `failed_before_frame`, or `cancelled`). `/playability/status` exposes only a rolling in-memory aggregate by outcome/failure stage; terminal records exclude title text, content IDs, URLs, provider payloads, and raw errors. Failures also carry a bounded `failure_kind` (`timeout`, `bot_check`, `blocked`, `format_unavailable`, `unavailable`, `mpv_handoff`, `other`) so resolve stalls are not logged as generic provider/resolve.
