...
**Debrid garbage safety (play path):** Prefer TorBox on same-hash ties (AIO dedup + service sort). Keep RD for unique cached releases (ladder steps admit both services; uncached RD stays excluded upstream). Proven-bad URLs (`debrid_copyright_block`, `debrid_status_clip`, `debrid_nfo_sidecar`) are session-blacklisted by `streamUrlHash` (~45 min) and skipped on later attempts — never blacklist a whole title. Preflight timeouts and `debrid_playback_unreadable` are **not** bad-cached (transient). Debrid VOD always probes with `vo=null` before visible handoff so copyright/status clips never become the stream the user watches. Couch errors map those failures to generic “streams are still preparing” copy — never surface “copyright infringement”.
