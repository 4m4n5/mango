...
**Branch:** `main` · **Roadmap:** [ROADMAP.md](ROADMAP.md) · **Acceptance:** [TESTING.md](TESTING.md)
...
- **Deploy wrappers fail closed.** `pi-deploy.sh` and `pi-exec-gate.sh`
  require `main`, a successful fetch, matching expected SHA,
  ...
  Source proof: `scripts/m6-ship/test-pi-deploy-hardening.sh`. The YouTube
  playback overhaul was deployed and targeted-gated at exact Pi SHA `28f2e1c`.
...
- `pi-deploy.sh`/`pi-exec-gate.sh` require `main`, a successful
