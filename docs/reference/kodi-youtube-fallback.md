...
> **Legacy path:** Enable with `MANGO_LEGACY_YOUTUBE=1`. Native direction uses mpv + catalog-service — not Kodi tiles for daily use. See [VISION.md](../VISION.md).

Context: [OPS.md](../OPS.md) · Gamepad: **D-pad** move · **B** (`304`) select · **Y** (`308`) back.
...
```bash
cd ~/mango && git pull
bash scripts/m1-foundation/pad/install-kodi-inputstream.sh
```
...
```bash
bash scripts/m1-foundation/pad/reset-kodi-youtube.sh
bash scripts/m1-foundation/pad/launch-kodi.sh
```
...
```bash
bash scripts/m1-foundation/pad/diagnose-kodi-youtube.sh
```
...
```bash
cd ~/mango && git pull
bash scripts/m1-foundation/pad/set-youtube-api-keys.sh
bash scripts/m1-foundation/pad/launch-kodi.sh
```
...
3. On Pi: `bash scripts/m1-foundation/pad/test-youtube-api-page.sh` — localhost and LAN should both return HTTP 200
...
```bash
killall kodi 2>/dev/null || true
bash scripts/m1-foundation/pad/kodi-enable-rpc.sh mango yourpassword
bash scripts/m1-foundation/pad/launch-kodi.sh
bash scripts/m1-foundation/pad/test-kodi-rpc.sh mango yourpassword
```
...
| **inputstream.adaptive 19.0.0 cannot be satisfied** | `bash scripts/m1-foundation/pad/install-kodi-inputstream.sh` → restart Kodi → install zip again |
...
| Sign in refreshes / nothing happens | GUI keys override file — `bash scripts/m1-foundation/pad/reset-youtube-login.sh`; expect device-code popup; sign in **twice** at [google.com/device](https://google.com/device) |
| D-pad dead in Kodi | `bash scripts/m1-foundation/pad/map-pro-controller.sh` then relaunch Kodi |
