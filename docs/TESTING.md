...
Evidence levels: [README.md](README.md). A green Reliability badge is
sampled machine health, not a release. Recorded device proof:
[STATUS.md](STATUS.md).
...
| Mac full | catalog `npm test` plus path-triggered suites | Deep local-pass | Pi behavior |
...
Live IPTV gates stay opt-in (`MANGO_LIVE_GATE=1`). YouTube playback
signoff uses `MANGO_YOUTUBE_PLAY=1`.
...
| `scripts/check-doc-links.py` / `scripts/check-public-surface.py` | PR |
...
| `scripts/m3-play/playability/gate-m3-library-grow.sh` | Mac full |
...
`scripts/gate-mango.sh` is the dispatcher.

## Couch acceptance

Record source SHA, Pi SHA, recommendation modes, TV/audio route, and
tester before starting. Verdicts: PASS, FAIL, DEFERRED, N/A.

Preconditions: Git-only deploy, idle couch, preserved dirty state, no
database wipes, no pairing-mode “fix”, no secret screenshots.
...
Do not inherit PASS from older screenshots. Update [STATUS.md](STATUS.md)
only after a named SHA observation.
