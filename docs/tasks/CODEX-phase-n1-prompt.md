...
1. Risks from N0 inventory (RAM headroom, Node on ARM)  
2. Spike order S0→S6 — which are already green vs blocked  
3. Pinned smoke title ID and why  
4. stremio-core failure fallback branch (if any)  
5. mpv socket path + singleton strategy  
6. Pad changes needed for `mpv` foreground  

**Do not** write `catalog-service` until **S0 (mpv HTTP)** and **S1 (stremio-core boot)** pass on the Pi.
...
- Secrets: `/etc/mango/stremio-export.json`, `/etc/mango/config.yaml`, keys — **never commit**.
- User must have Stremio export on Pi before S1. If missing, stop with clear instructions; do not fake addons.
...
```
S0  bash scripts/phase-n1/spike-mpv-http.sh
S1  bash scripts/phase-n1/spike-stremio-core.sh
...
- `scripts/phase-n1/spike-mpv-http.sh`
- `scripts/phase-n1/spike-stremio-core.sh`
- `scripts/phase-n1/mpv-play.sh` · `mpv-stop.sh` · `mpv-ipc.sh`
- `scripts/phase-n1/gate-n1-smoke.sh` — master gate; exit non-zero on failure

**New service (minimum):**

- `src/catalog-service/` — Node 20+ TypeScript
- `@stremio/stremio-core-web` for addon graph
...
- `scripts/phase0/mango-tv-pad.py` — `foreground_app` mpv; ⌂ → stop mpv + launcher

**Docs (minimum):**

- `docs/N1-INVENTORY.md` — plan, spike logs, TTFF, pinned title, waivers
- `config/stremio-export.example.json`
- Update `FOREGROUND.md`, `AGENTS.md`
...
3. S0/S1 — Pi spikes only; commit spike scripts when green
4. IMPLEMENT — catalog-service + mpv helpers + pad
5. COMMIT + PUSH — feat/native-experience
6. DEPLOY Pi — git pull; npm ci && build in catalog-service
...
11. COMMIT + PUSH — inventory + any fixes
```
...
- mpv IPC: [`DOCS/man/ipc.rst`](https://github.com/mpv-player/mpv/blob/master/DOCS/man/ipc.rst) — JSON over unix socket.
...
| `gate-n0.sh` | exit 0 |
| Pad `foreground_app` on launcher | `launcher` (regression) |

### mpv reference launch

```bash
mpv --fs --idle=yes --input-ipc-server="${HOME}/.cache/mango/mpv.sock" \
  --hwdec=auto-safe 'https://…'
```
...
(Replace ID with pinned title from inventory if different.)
...
test -f /etc/mango/stremio-export.json || echo "STOP: paste Stremio export first"
...
Think first: N1-INVENTORY.md §Plan before feature code. Spikes S0→S6 in order.
Build catalog-service (:3020) + mpv IPC + POST /play; pad mpv + ⌂. No launcher rails UI.
SSH Pi (mango), run gate-n1-smoke.sh + gate-n0.sh. Git-only deploy. stremio-core on ARM must be proven — no Stremio desktop gate pass.
```
