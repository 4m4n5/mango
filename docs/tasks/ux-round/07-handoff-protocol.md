...
| `--panel-edge-fade` (3rem) | 2–4.5rem | How far stream/episode rows dissolve at the top and bottom of the side panel. It is a mask, not a scrim, so the rows fade into the background rather than toward a colour — and `updateEdgeFade()` caps each band by the content actually hidden on that side, so the extremes and short lists get no fade at all. Raise it if the cut still reads as an edge from the sofa; lower it if it swallows a row you were choosing between. Do not replace it with an overlay gradient: that draws the box, which is the defect it was built to remove. |
