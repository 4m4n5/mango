...
| Library | Mango-owned Continue, Saved, history, finished state, Fire/Water ratings, feedback, attribution, and normalized YouTube history. Current source adds canonical Saved placement through tab-only migration 18; Pi migration/readback and the Dune-from-TV-Search couch check are deferred |
...
| YouTube recommendations | Current source advances to v2.7 quality-gated reservoirs of up to 512 titles per rail and independent weighted cache-only X; it is not Pi-deployed. A later read-only `4a175197` v2.6 observation found generation 21, 480 candidates in four 120-title reserves, 999 active provenance rows, 55 subscriptions, and 2,548 Takeout anchors. `fb20baa` remains the last fully gated proof; exported Search history remains unused |
