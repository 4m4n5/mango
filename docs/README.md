...
| VOD recommendations | `93345e8` is latest-only and Pi-served: deterministic `vod-content-profile-v2`, immutable compatible StoryDNA overlays, local Household Story Frontier ranking, cached six-card dealing, optional exact-ID TMDB, and an off-by-default bounded frontier. Complete accounting, 224/230 reserves, cached X, memory, and launch gates pass; human thematic judgment remains open |
| YouTube recommendations | `93345e8` is latest-only and Pi-served: authoritative subscription/history acquisition and local v2 generations. Current history-only For You/Beyond reserves are healthy, provenance is clean, and More Like is conditionally omitted as `not_applicable`; human relevance/focus/launch observation remains open |
