...
| VOD recommendations | `772b3d5` is latest-only: deterministic `vod-content-profile-v2`, immutable compatible StoryDNA overlays, local Household Story Frontier ranking, cached six-card dealing, optional exact-ID TMDB, and an off-by-default bounded frontier. `off` exposes no For You rail; `shadow` builds with exact Household Saved and no public epoch; `serve` requires a promotion-eligible active generation. Shuffle, active-pointer, migration-preservation, and schema-diagnostic contracts have focused Mac tests; Pi promotion and couch proof remain open |
| YouTube recommendations | `772b3d5` is latest-only: authoritative subscription/history acquisition and local v2 generations. `off` returns exact active-profile History/Saved without the former 409; `shadow` hides Household recommendation rails; `serve` exposes only v2. Mac mode/ownership tests pass; account-specific Pi promotion and couch proof remain open |
