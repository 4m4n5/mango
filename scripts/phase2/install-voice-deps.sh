...
if command -v apt-get >/dev/null 2>&1; then
  ...
  brew install mkcert
  echo "Install Piper/Whisper Python deps with: bash scripts/phase2/install-orchestrator-deps.sh"
else
...
echo "→ Piper voice: bash scripts/phase2/download-piper-voice.sh"
