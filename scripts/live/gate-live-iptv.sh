...
#   bash scripts/live/install-nexotv-free.sh   # optional free tier
#   bash scripts/live/nexotv-config.sh apply-area69   # or apply iptv-org-sports
...
if python3 "$REPO_DIR/scripts/live/discover-sports-channels.py" \
...
if bash scripts/m2-catalog/service/mpv-play.sh \
  --url "$PLAY_URL" \
  --probe \
  --live \
...
if [[ "${MANGO_LIVE_PLAY:-0}" == "1" ]]; then
  ...
  bash scripts/m2-catalog/service/mpv-play.sh --url "$PLAY_URL" --live --timeout-ms 12000 --min-duration-sec 3 &
  sleep 8
  bash scripts/m2-catalog/service/mpv-stop.sh >/dev/null 2>&1 || true
