...
#   bash scripts/live/nexotv-config.sh apply [profile-id]
#   bash scripts/live/nexotv-config.sh list-profiles
#   bash scripts/live/nexotv-config.sh wire-export
#   bash scripts/live/nexotv-config.sh manifest
#
...
apply_free() {
  ...
  nexotv_free_health_ok || die "NexoTV free down at $free_base — run: bash scripts/live/install-nexotv-free.sh"
  mkdir -p "$data_dir"
...
apply_news() {
  ...
  nexotv_news_health_ok || die "NexoTV news down at $news_base — run: bash scripts/live/install-nexotv-news.sh"
  mkdir -p "$data_dir"
...
apply_cartoons() {
  ...
  nexotv_cartoons_health_ok || die "NexoTV cartoons down at $cartoons_base — run: bash scripts/live/install-nexotv-cartoons.sh"
  mkdir -p "$data_dir"
