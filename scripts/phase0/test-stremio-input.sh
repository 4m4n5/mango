...
# Run on the Pi: bash scripts/phase0/test-stremio-input.sh
...
if ! pgrep -x stremio >/dev/null 2>&1 && ! pgrep -f '/opt/stremio' >/dev/null 2>&1; then
  echo "! Stremio not running — bash scripts/phase0/reset-stremio.sh"
  exit 1
fi
...
echo "  If nothing moved, run: bash scripts/phase0/map-pro-controller.sh"
echo "  Then: bash scripts/phase0/focus-stremio.sh"
