...
# Run on the Pi: bash scripts/phase0/install-kodi-inputstream.sh
...
if ! command -v kodi &>/dev/null && ! command -v kodi21 &>/dev/null; then
  echo "! Kodi not installed. Run: bash scripts/phase0/install-kodi.sh"
  exit 1
fi
...
echo
echo "Next: bash scripts/phase0/reset-kodi-youtube.sh  then install the YouTube zip"
