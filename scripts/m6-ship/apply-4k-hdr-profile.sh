#!/usr/bin/env bash
# Apply or revert Mango's M6.3 target-TV playback profile on the Pi.
...
apply       enable target-TV 4K stream policy and unified VLC couch playback
...
remove_env_keys() {
  ...
  touch "$VOICE_ENV"
  out="$(mktemp)"
  grep -vE '^export (MANGO_CATALOG_FILTERS|MANGO_LAUNCHER_DISPLAY_MODE|MANGO_LAUNCHER_DISPLAY_RATE|MANGO_MPV_DISPLAY_MODE|MANGO_MPV_DISPLAY_RATE|MANGO_MPV_DISPLAY_RATE_STRICT|MANGO_MPV_DISPLAY_FALLBACK_MODE|MANGO_MPV_DISPLAY_FALLBACK_RATE|MANGO_MPV_HWDEC|MANGO_MPV_MATCH_REFRESH|MANGO_MPV_MATCH_4K_MODE|MANGO_MPV_VIDEO_SYNC|MANGO_MPV_INTERPOLATION|MANGO_PLAYBACK_BACKEND|MANGO_VLC_DISABLE_XCOMPMGR|MANGO_VLC_STOP_LAUNCHER|MANGO_VLC_AOUT|MANGO_VLC_ALSA_DEVICE|MANGO_PREFERRED_HDR_TAGS|MANGO_PREFERRED_VIDEO_CODECS)=' \
    "$VOICE_ENV" >"$out" || true
  mv "$out" "$VOICE_ENV"
...
case "$cmd" in
  apply)
    ...
    append_env MANGO_MPV_DISPLAY_MODE "1920x1080"
    ...
    append_env MANGO_VLC_DISABLE_XCOMPMGR "1"
    append_env MANGO_VLC_STOP_LAUNCHER "1"
    append_env MANGO_VLC_AOUT "alsa"
    append_env MANGO_VLC_ALSA_DEVICE "hdmi:CARD=vc4hdmi0,DEV=0"
