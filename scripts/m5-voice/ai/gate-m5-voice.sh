...
if bash scripts/m5-voice/ai/validate-stt-hinglish-config.sh; then
  ok hinglish-stt-config
else
  bad hinglish-stt-config
fi

if bash scripts/m5-voice/ai/validate-voice-tv-open.sh; then
  ok voice-tv-open-ack
else
  bad voice-tv-open-ack
fi

if bash scripts/m5-voice/ai/validate-voice-orchestrator-open.sh; then
  ok voice-orchestrator-open-ack
else
  bad voice-orchestrator-open-ack
fi

if bash scripts/m5-voice/ai/validate-voice-title-switch.sh; then
  ok voice-title-switch-without-home
else
  bad voice-title-switch-without-home
fi

if bash scripts/m5-voice/ai/gate-m5-conversation-policy.sh; then
  ok n5c-conversation-policy
else
  bad n5c-conversation-policy
fi

if bash scripts/m5-voice/ai/gate-m5-gardener.sh; then
  ok n5c-gardener
else
  bad n5c-gardener
fi

if bash scripts/m5-voice/ai/gate-m5-companion-llm-policy.sh; then
  ok n5c-companion-llm-policy
else
  bad n5c-companion-llm-policy
fi

if bash scripts/m5-voice/ai/gate-m5-ai-bootstrap.sh; then
  ok n5d-ai-catalog-bootstrap
else
  bad n5d-ai-catalog-bootstrap
fi

if bash scripts/m5-voice/ai/gate-m5-mdblist-reserve.sh; then
