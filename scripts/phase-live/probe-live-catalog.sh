...
python3 "$REPO_DIR/scripts/phase-live/discover-sports-channels.py" \
...
if $PLAY; then
  exec bash scripts/phase-n1/mpv-play.sh --url "$PLAY_URL" --live --timeout-ms 60000 --min-duration-sec 5
else
  exec bash scripts/phase-n1/mpv-play.sh --url "$PLAY_URL" --probe --live --timeout-ms 45000
fi
