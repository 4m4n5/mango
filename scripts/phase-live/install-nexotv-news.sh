#!/usr/bin/env bash
# Install or upgrade Pi-local NexoTV news tier (IPTV-org news on :7002).
...
COMPOSE_DIR="$REPO_DIR/deploy/nexotv-news"
...
for _ in $(seq 1 60); do
  if curl -sf --max-time 3 http://127.0.0.1:7002/health >/dev/null; then
    echo "NexoTV news ready: http://127.0.0.1:7002/configure"
    exit 0
  fi
  sleep 1
done

docker_compose logs --tail=80 nexotv-news >&2 || true
echo "NexoTV news did not become healthy at /health" >&2
exit 1
