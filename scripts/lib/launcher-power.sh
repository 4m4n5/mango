...
launcher_restart_for_clean_gl() {
  # Thaw first so systemd clears FreezerState. Snapshot durability is owned by
  # the launcher SPA (it must not clear playback-return on thaw refresh).
  # Kill-while-frozen can leave the unit permanently FreezerState=frozen with
  # no ControlGroup, blocking later start/stop until user-manager recovery.
  launcher_thaw
