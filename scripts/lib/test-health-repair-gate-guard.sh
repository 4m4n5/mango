...
# The gate/curl pkills must sit behind the intentional-gate / playback guards.
python3 - "$REPAIR" <<'PY'
...
# Ensure gate pkills are after the early returns.
gate_idx = body.find("pkill -f 'gate-m3-verified-rails'")
curl_idx = body.find("pkill -f 'curl.*127\\.0\\.0\\.1:3020/play'")
guard_idx = body.find("intentional_pre_couch_gate_active")
if gate_idx < 0 or curl_idx < 0:
    raise SystemExit("FAIL: expected gate/curl pkills still present for orphan cleanup")
if not (0 <= guard_idx < gate_idx and guard_idx < curl_idx):
    raise SystemExit("FAIL: gate/curl pkills are not guarded by intentional_pre_couch_gate_active")
print("PASS: health-repair preserves intentional pre-couch gates")
PY
