...
# Stack/UI start owns the boot wake. A pad press keeps its lower-latency wake
# path, but refresh must recover an already-Off DPMS panel without input.
bash scripts/lib/mango-display-wake.sh 2>/dev/null || true
