...
# Usage: bash scripts/m1-foundation/pad/test-kodi-rpc.sh <username> <password>
...
if [[ -z "$USER" || -z "$PASS" ]]; then
  ...
  echo
  echo "Enable first: bash scripts/m1-foundation/pad/kodi-enable-rpc.sh <user> <pass>"
  exit 1
fi
...
if ! pgrep -x kodi >/dev/null 2>&1 && ! pgrep -f 'kodi.bin' >/dev/null 2>&1; then
  echo "! Kodi is not running — run: bash scripts/m1-foundation/pad/launch-kodi.sh"
  exit 1
fi

if ! ss -tln 2>/dev/null | grep -q ":${PORT} "; then
  echo "! Nothing listening on port ${PORT}"
  echo "  Run: bash scripts/m1-foundation/pad/kodi-enable-rpc.sh ${USER} '<password>'"
  echo "  Then: killall kodi && bash scripts/m1-foundation/pad/launch-kodi.sh && sleep 5"
...
case "$HTTP_CODE" in
  ...
  401)
    ...
    ;;
  000)
    echo "✗ Could not connect — enable RPC: bash scripts/m1-foundation/pad/kodi-enable-rpc.sh"
    exit 1
    ;;
  *)
