...
# Run on the Pi:
#   bash scripts/m1-foundation/pad/map-gamepad-ssh.sh
#   bash scripts/m1-foundation/pad/map-gamepad-ssh.sh --device /dev/input/event6 --name "8BitDo Pro 2"
#   bash scripts/m1-foundation/pad/map-gamepad-ssh.sh --buttons-only   # A/B only, keep existing D-pad preset
...
if [[ -z "$EVENT_DEV" ]]; then
  EVENT_DEV=$(find_gamepad_event) || {
    echo "No gamepad found. Pair 8BitDo: bash scripts/m1-foundation/pad/setup-8bitdo-bt.sh"
    exit 1
  }
fi
