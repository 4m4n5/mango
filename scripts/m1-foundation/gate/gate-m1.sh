...
mango_gate_init
gate_header "mango M1 foundation gate"
...
if [[ "${MANGO_VOICE:-0}" == "1" ]]; then
  if curl -skf --max-time 3 https://127.0.0.1:8765/health >/dev/null; then
    ...
    gate_fail "orchestrator down — bash scripts/m5-voice/stack/start-voice-stack.sh"
  fi
  ...
  if [[ "${MANGO_GATE_WS_STRESS:-0}" == "1" ]]; then
    python3 scripts/m1-foundation/gate/ws-stress.py --url wss://127.0.0.1:8765/ws --count 10 --insecure \
