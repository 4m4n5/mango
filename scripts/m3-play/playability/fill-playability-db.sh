...
#
# Usage:
#   bash scripts/m3-play/playability/fill-playability-db.sh
#
# Env:
...
bash scripts/m3-play/playability/playability-maintenance.sh --mode grow --bootstrap
...
if [[ "$POOL_TOPUP" == "1" ]]; then
  ...
  export MANGO_MAINTENANCE_SKIP_GATE=1
  export MANGO_PLAYABILITY_CANDIDATE_LIMIT="${MANGO_FILL_CANDIDATE_LIMIT:-250}"
  bash scripts/m3-play/playability/playability-maintenance.sh --mode grow
fi
...
echo "optional: bash scripts/m4-addons/gate-m4-catalogs.sh"
