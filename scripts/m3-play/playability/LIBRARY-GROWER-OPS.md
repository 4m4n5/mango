...
Automatic VOD Play confirms a clean or proven-transient error-only aggregate empty at most twice by default (`MANGO_STREAM_ZERO_RETRY_ATTEMPTS=2`, `MANGO_STREAM_ZERO_RETRY_DELAY_MS=1200`). Both confirmations share the exact-title/episode single flight and deadline; Detail `GET /stream`, Live, and picker refresh use zero. HTTP failures, 429s, permanent provider/account errors, authoritative no-stream placeholders, invalidated work, and exhausted deadlines are not retried. Grow verification separately retries a zero-stream resolve once by default (`MANGO_PLAYABILITY_VERIFY_ZERO_RETRY_ATTEMPTS=1`, `MANGO_PLAYABILITY_VERIFY_ZERO_RETRY_DELAY_MS=1200`) before writing a `no_stream` tombstone. Non-grow background verification keeps zero retries unless explicitly configured.
