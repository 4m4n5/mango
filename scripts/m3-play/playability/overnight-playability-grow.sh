...
#
# Usage:
#   bash scripts/m3-play/playability/overnight-playability-grow.sh --detach   # start on Pi, go to sleep
#   bash scripts/m3-play/playability/overnight-playability-grow.sh --status   # tail progress
#   bash scripts/m3-play/playability/overnight-playability-grow.sh            # foreground (debug)
#
# Env:
...
#   MANGO_PLAYABILITY_FRESH_PER_RAIL   fresh probes queued per rail (default 40)
...
restore_couch() {
  ...
  bash "$REPO_DIR/scripts/m3-play/playability/mpv-probe-pool.sh" stop-all >/dev/null 2>&1 || true
...
preflight() {
  ...
  # shellcheck source=../../m4-addons/lib/aiometadata.sh
...
run_chunk() {
  ...
  npm --prefix "$REPO_DIR/src/catalog-service" exec tsx -- \
    "$REPO_DIR/scripts/m3-play/playability/playability-indexer.ts" \
