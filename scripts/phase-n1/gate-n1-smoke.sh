...
bash scripts/phase-n1/check-n1-prereqs.sh >/dev/null && gate_pass "prereqs" || gate_fail "prereqs"

if [[ "${MANGO_GATE_SPIKES:-0}" == "1" ]]; then
  bash scripts/phase-n1/spike-mpv-http.sh >/dev/null && gate_pass "spike-mpv-http" || gate_fail "spike-mpv-http"
  bash scripts/phase-n1/spike-stremio-core.sh >/dev/null && gate_pass "spike-stremio-core" || gate_fail "spike-stremio-core"
fi
