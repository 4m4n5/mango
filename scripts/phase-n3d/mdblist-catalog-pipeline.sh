...
#   bash scripts/phase-n3d/mdblist-catalog-pipeline.sh sync-curated
#   bash scripts/phase-n3d/mdblist-catalog-pipeline.sh export-llm [--tag comedy]
#   bash scripts/phase-n3d/mdblist-catalog-pipeline.sh check-import
#   bash scripts/phase-n3d/mdblist-catalog-pipeline.sh compose plan proposal.json
#   bash scripts/phase-n3d/mdblist-catalog-pipeline.sh compose apply proposal.json [--write]
#   bash scripts/phase-n3d/mdblist-catalog-pipeline.sh measure
#   bash scripts/phase-n3d/mdblist-catalog-pipeline.sh couch-measure   # Pi: probe + inventory + export
#
# LLM workflow:
...
run_check_import() {
  ...
  if [[ ! -f "$import_json" ]]; then
    ...
    echo "  then: bash scripts/phase-n3d/aiometadata-config.sh check $import_json" >&2
    return 1
  fi
  MANGO_CATALOG_YAML="$CATALOG_YAML" bash "$REPO_DIR/scripts/phase-n3d/aiometadata-config.sh" check "$import_json"
}
...
case "$cmd" in
  ...
  compose)
    ...
    python3 "$REPO_DIR/scripts/phase-n3d/rail-compose.py" "$sub" "$proposal" "${extra[@]}"
    ;;
  full)
    run_sync 0
    run_export_llm
